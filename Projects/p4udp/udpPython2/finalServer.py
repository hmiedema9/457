...
def removeAndSlideElements(w, confirmedAcks, f, sp, es, ms):
    ...
    removedElms = 0
    print "Window packets before setAcks:"
    print str(wkeys)
    print "Packet sq # before setAcks:"
    ...
    for x in confirmedAcks:
        ...
        if packetConfirmed:
            ...
            if x == (lowestWindowKey):
                ...
                del w[x]
                wkeys.remove(x)
                print "Removed " + str(x) + " from window"
                print wkeys
                removedElms += 1
    ...
    for i in range(0, removedElms):
        ...
        chunk = f.read(es)
        w[sp] = (chunk, False)
        print "Add packet " + str(sp)
        print "Window size: " + str(len(w))
	print "sequence # " + str(sp)
...
while True:
    ...
    if os.path.exists(f):  # check if the file exists
        ...
        print "Size of requested file (in bytes): " + str(fileSize)
        ...
        with open(f, 'rb') as f:
            ...
            while fileIncomplete:
                ...
                print "Listening for acknowledgement"
